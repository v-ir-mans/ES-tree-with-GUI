...
def read_excel_to_entries(path, sheet_name=0):
    ...
    for row in sheet.iter_rows(min_row=2, values_only=True):
        # Extract the first and last columns as id and klase
        id_key, id = columns[0], row[0]
        klase_key, klase = columns[-1], row[-1]

        # Create a dictionary of other columns except id and klase
        other_columns_dict = {columns[i]: row[i] for i in range(len(row)) if columns[i] != id_key and columns[i] != klase_key}

        # Convert the entire row to a dictionary with all values as their original types
        full_row = {columns[i]: row[i] for i in range(len(row))}
